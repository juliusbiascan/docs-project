    ...
    <DropdownMenuContent className="p-1 flex flex-col gap-y-1">
      {lineHeights.map(({ label, value }) => (
        <button
          ...
          <span className="ext-sm">{label}</span>
        </button>
      ))}
    </DropdownMenuContent>
  </DropdownMenu>)
    ...
    <div className="flex items-center gap-x-0.5">
      ...
      {isEditing ? (
        <input
          ...
          className="h-7 w-10 text-sm text-center shrink-0 border border-neutral-400 rounded-sm bg-transparent focus:outline-none focus:ring-0" />
      ) : <button
        onClick={() => {
          setIsEditing(true);
          setFontSize(currentFontSize);
        }}
        className="h-7 w-10 text-sm text-center shrink-0 border border-neutral-400 rounded-sm bg-transparent cursor-text">
        {currentFontSize}
      </button>}
      <button className="h-7 w-7 shrink-0 flex items-center justify-center rounded-sm hover:bg-neutral-200/80"
        onClick={increment}>
...
const ListButton = () => {
  ...
  const list = [
    ...
    },
    {
      ...
      isActive: () => editor?.isActive("orederedList"),
    ...
    <><DropdownMenu >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          ...
          <Input placeholder="Inser Image URL" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} onKeyDown={(e) => {
            if (e.key === "Enter") {
...
export const Toolbar = () => {
  ...
  return <div className="bg-[#F1F4F9] px-2.5 py-0.5 rounded-[24px] min-h-[40px] flex item-center gap-x-0.5 overflow-x-auto">
    {sections[0].map((item => (
